...
const CommentForm = ({parentId,showForm,hideForm}) => {
    // console.log(parentId);
    const [hidden,setHidden] = useState(showForm);
    const filesInput = document.querySelector('.comment-file-input');
    const commentInput = document.querySelector('#comment');
    const postUrl = `http://localhost:8000/posts/${parentId}`;
    ...
    return ( 
        <div className="add-comment-form">
            ...
            {/* form starts */}
             <div className = {`comment-container float-form ${hidden} `}>
                <form action="" method="get" className="comment-form relative float-form" id="comment-form">
                 ...
                 <div className="form-header">
                         <div className="cancel inline" onClick={displayDialog}><i className="fa-solid fa-xmark i-size"></i></div>
                         <div className="back inline" onClick={displayDialog}><i className="fa-solid fa-arrow-left i-size"></i></div>
                     </div>
