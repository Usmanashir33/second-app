...
const PostCard = ({object,ishover,cardType}) => {
    const {id,body,likes,shares,reposts,views,date,postcomments,rep} = object;
    const {first_name,last_name,username} = object.user? object.user : {};
    const history = useHistory();
    const { setCommentFormType,showCForm,setShowCForm,setClickedObjectId} = useContext(dataRendingContext)
