...
const CommentForm = ({data,setData}) => {
    ...
    if(commentFormType == 'post'){
        const {dopost:sendData} = usePostData(`/posts/post/77/comments/`,data,setData)
    }else if (commentFormType == 'comment'){
        const {dopost:sendData} = usePostData(`commenturl`)
    }
    const displayDialog = () => {
        ...
        <div className="add-comment-form">
            ...
            {/* form starts */}
             <div className = {`comment-container float-form`}>
                <form onSubmit={(e) => {handleSubmit}} className="comment-form relative float-form relative" id="comment-form">
