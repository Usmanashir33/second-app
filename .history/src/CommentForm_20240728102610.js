...
const CommentForm = ({type}) => {
    const {clickedObjectId,commentFormType,setCommentFormType} = useContext(dataRendingContext);
    ...
    console.log(commentFormType);
    console.log(parent);
    console.log(post);
    console.log(comment);

    // const Url = type === "comment"? `/posts/post/${clickedObjectId}/comments/`: `;
    ...
    useEffect(() => {
        if (commentFormType == 'post'){
            fetchPost( `/posts/post/${clickedObjectId}/comments/`);
            setParent(post);
        }else{
            fetchComment(Url);
            setParent(comment);
                         ...
                         <div className="post-col2">
                             <div className="post-header">
                                 <div className="user-data">
                                    ...
                                    <span className="post-username in-form-username">{parent && parent}</span>
                             ...
                             <div className="replying-to">
                                 <span className="small-txt muted-txt">Replaying to </span><span className="in-form-username-link">
                                     <Link to={`profile/${parent && parent.id}/`} className="simple-link small-txt">{parent && parent.user.username} </Link>
                                 </span>
                             </div>
                         </div>
                     </div>
