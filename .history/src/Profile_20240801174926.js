...
const Profile = () => {
    ...
    const [followings,setFollowings] = useState(
        (user && user.followings)? user.followings.length : ''
    )
        ...
        <div className="profile main-page ">
            ...
            <div className="user-buttons">
               ...
               {(!myProfile && !followed) && <Link to ="#" className="simple-link"  >
                    <button type="button" onClick={(e) => {followUser(`/accounts/user/${pk}/follow/`)}} className="user-btn">follow</button>
                </Link>}
               {(!myProfile && followed) && <Link to ="#" className="simple-link"  >
                    <button type="button" onClick={(e) => {followUser(`/accounts/user/${pk}/follow/`)}} className="followed">unfollow</button>
                </Link>}
            ...
            <div className="user-info">
                ...
                <div className="follow">
                    <span className="following "> user.followings.length </span>Followings 
                    <span className="followers ml-10">{ user && user.followers.length}</span> Followers
                </div>
            </div>
