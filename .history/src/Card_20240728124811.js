...
const PostCard = ({object,ishover,cardType}) => {
    const {id,body,likes,shares,reposts,views,date,postcomments} = object;
    const {first_name,last_name,username} = object.user? object.user : {};
    const history = useHistory();
    const { setCommentFormType,setShowCForm,setClickedObjectId} = useContext(dataRendingContext)
        ...
        <div className="post-card" key={id} onClick={(e) => cardClicked(e)}>
                ...
                <div className="post-col2">
                    ...
                    <div className="postnav">
                                <Link className='post-nav-link' id ="comment-${id}" onClick={(e) => {
                                    e.stopPropagation();
                                    commentClick(id);
                                }}>
                                    <div className="" >
                                        {/* <FontAwesomeIcon {comments} */}
                                        <FontAwesomeIcon icon={faComment} className="comment" />{ postcomments && postcomments.length}
                                    </div>
                                </Link>
