...
const UserCard = ({object}) => {
    ...
    console.log(currentUser.followers,id);
    const cardClicked = () => {
    ...
    useEffect(() => {
        if (object){
            ...
            setFollowedMe(
                currentUser ? currentUser.followers.includes(currentUser.id) : false
            )
        }
    },[])
    return ( 
        <div className="post-card user-card" key={id} onClick={(e) => cardClicked(e)}>
                <div className="post-col1 relative">
