...
const CommentDetail = () => {
    ...
    const {comment,fetchComment,showCForm,showUForm} = useContext(dataRendingContext)
    const {replies,setReplies,fetchReplies} = useContext(dataRendingContext);
    const {doDelete} = useDeleteDat(comments,setComments);
    ...
    return (
        <div className="post-detail-page relative">
           <header className="flex center-v profile-header">
            ...
            <div className="mt-45">
                {comment && <PostCard object = {comment} doDelete={doDelete} ishover ='no' cardType='comment' />}
            </div>
            ...
            {showUForm && <UpdateForm  data={replies} setData={setReplies}/> }
