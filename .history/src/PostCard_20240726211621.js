...
const PostCard = ({post,postLink,ishover,commentClick}) => {
    const {id,body,like,shares,reposts,views,date,comments} = post;
                ...
                <div className="post-col2">
                    <div className="post-header">
                                <div className="post-userdata">
                                    <span className="post-name">{post.user.first_name} {post.user.last_name}</span>
                                    <span className="post-username">@{post.user.username}</span>
                                    <span className="post-date"> {post.date} </span>
                                </div>
                                <div className="post-dot" onClick={(e) => {showDotDD(e,`dot-${id}`,`${post.user.username}`)}}>
