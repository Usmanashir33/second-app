...
const PostCard = ({post,postLink,ishover,commentClick}) => {
    const {id,body,like,shares,reposts,views,date,comment} = post;
    ...
    return ( 
        <div className="post-card" key={id} onClick={(e) => postLink(e,id)}>
                <div className="post-col1">
                    <Link to={`/profile/${post.user.id}`} className='child-link'>
                ...
                <div className="post-col2">
                    <div className="post-header">
                                <div className="post-userdata">
                                    <span className="post-name">{first_name} {last_name} </span>
                    ...
                    <div className="postnav">
                                <Link to="##" className='post-nav-link' id ="comment-${id}" onClick={(e) => {
                                    ...
                                    <div className="" >
                                        ...
                                        <FontAwesomeIcon icon={faComment} className="comment" />{comment}000
                                    </div>
                                </Link>

                                <Link to="##" className='post-nav-link child-link' id ="repost-${id}">
                                    <div className="" >
                                    <FontAwesomeIcon icon={faRotate} className="comment" />{reposts.lengt}
                                    </div>
                                </Link>

                                <Link to="##" className='post-nav-link child-link' id ="like-${id}" >
                                    <div className="like" >
                                    <FontAwesomeIcon icon={faHeart} className="comment" />{like.length}
                                ...
                                <Link to="##" className='post-nav-link child-link' id ="view-${id}">
                                    <div className="view" >
                                    <FontAwesomeIcon icon={faEye} className="comment" />{comment}000
                                    </div>
                                </Link>
